...
public class NavXStraightDriving extends BaseAutoOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        front_left.setMode(RUN_USING_ENCODER);
        front_right.setMode(RUN_USING_ENCODER);
        rear_left.setMode(RUN_USING_ENCODER);
        rear_right.setMode(RUN_USING_ENCODER);
            ...
            if ( yawPIDController.waitForNewUpdate(yawPIDResult, DEVICE_TIMEOUT_MS ) ) {
                if ( yawPIDResult.isOnTarget() ) {
                    ...
                    if ( output < 0 ) {
                        ...
                        front_left.setPower(drive_speed - output*2);
                        front_right.setPower(drive_speed - output*2);
                        rear_left.setPower(drive_speed + output*2);
                        rear_right.setPower(drive_speed + output*2);
                    } else {
                        /* Rotate Right */
                        telemetry.addData("PID",yawPIDResult.getOutput());
                        front_left.setPower(drive_speed + output*2);
